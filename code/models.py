...
class LFS_Head(nn.Module):
    ...
    def forward(self, x):
        # turn RGB into grayscale with standard weights
        ...
        x_unfold = self.unfold(x)   # [N, C * S * S, L]   L:no of blocks
        L = x_unfold.size()[2]
...
class F3Net(nn.Module):
    def __init__(self, num_classes=1, img_width=299, img_height=299, LFS_window_size=10, LFS_stride=2, LFS_M = 6, mode='Mix', device=None):
        ...
        if mode=='Mix':
            self.mix_block = MixBlock(2048, img_width, img_height)
    ...
    def forward(self, x):
        ...
        if self.mode=='Mix':
            ...
            y_FAD = self._norm_fea(y_FAD)
            y_LFS = self._norm_fea(y_LFS)
            y = torch.cat((y_FAD, y_LFS), dim=1)
